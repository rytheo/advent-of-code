...
impl Intersect {
    ...
    fn cost(&self) -> i32 {
        (self.x - self.h.start.0).abs() + self.h.prev
        + (self.y - self.v.start.1).abs() + self.v.prev
    }
...
fn main() {
    ...
    let wires: Vec<_> = text.lines().map(|line| {
        ...
        for step in line.split(',') {
            ...
            let line = Line { start: (x0, y0), end: (x1, y1), prev };
            match &step[..1] {
    ...
    println!("Part 1: {}", intersections.iter().map(|i| i.dist()).min().unwrap());
    println!("Part 2: {}", intersections.iter().map(|i| i.cost()).min().unwrap());
}
