...
impl CPU {
    pub fn new(program: &[Integer]) -> CPU {
        CPU {
            mem: program.iter().enumerate().map(|(i, v)| (Integer::from(i), v.clone())).collect(),
            ip: Integer::new(),
            ro: Integer::new(),
            input: VecDeque::new(),
            output: VecDeque::new(),
    ...
    pub fn run(&mut self) -> u32 {
        loop {
            ...
            match op {
                ...
                9 => { // Change relative offset
                    let p = self.get_params(1, &modes, false);
                    self.ro += &p[0];
                    self.ip += 2;
                }
                1 | 2 | 7 | 8 => { // Combine two values
                    let mut p = self.get_params(3, &modes, true);
                    let c = p.pop().unwrap();
                    let (a, b) = (&p[0], &p[1]);
                    self.mem.insert(c, match op {
                        1 => Integer::from(a + b),
                        2 => Integer::from(a * b),
                        7 => Integer::from(a < b),
                        8 => Integer::from(a == b),
    ...
    fn get_op(&self) -> (u32, [u32; 3]) {
        ...
        (inst % 100, [inst / 100 % 10, inst / 1000 % 10, inst / 10000])
    }

    fn get_params(&self, n: usize, modes: &[u32], write: bool) -> Vec<Integer> {
        (0..n).map(|i| {
            let mut p = self.mem.get(&(self.ip.clone() + 1 + i as u8)).cloned().unwrap_or_default();
